...
def normalize_ocr_json(payload: Any) -> Dict[str, Any]:
    ...
    embedded_payloads = _extract_embedded_json_payloads(payload)
    structured_sources = embedded_payloads or [payload]

    fragments: List[str] = []
    for source in structured_sources:
        fragments.extend(_extract_indicator_lines(source))
        fragments.extend(_extract_structured_lines(source))
...
def split_ocr_request_payload(payload: Dict[str, Any]) -> Tuple[Any, Dict[str, Any]]:
    ...
    options = {key: payload.get(key) for key in option_keys if key in payload}
    if options:
        diagnosis_payload = {key: value for key, value in payload.items() if key not in option_keys}
        return diagnosis_payload, options
    return payload, {}
...
def _extract_text_fragments(value: Any) -> List[str]:
    ...
    if isinstance(value, str):
        ...
        if stripped and _parse_json_object_from_text(stripped) is None:
    ...
    if isinstance(value, dict):
        for key, item in value.items():
            ...
            if key_l in TEXT_KEYS and isinstance(item, str):
                if _parse_json_object_from_text(item) is None:
...
def _extract_embedded_json_payloads(value: Any) -> List[Dict[str, Any]]:
    ...
    if isinstance(value, dict):
        for item in value.values():
            if isinstance(item, (str, dict, list, tuple)):
                payloads.extend(_extract_embedded_json_payloads(item))
...
def _extract_json_object_text(text: str) -> str:
    ...
    if lower.startswith("raw text"):
        stripped = stripped[len("raw text") :].strip()
