...
from engines.agents.llm_mdt_agent import run_llm_mdt_agent
...
class DiagnosisPipeline:
    ...
    async def run(
        ...
        mdt_report: Dict[str, Any] = {"enabled": False, "source": "llm_mdt", "reason": "disabled"}
        if use_multi_agent:
            for row in agent_outputs:
                row.setdefault("source", "rule_agent")
            if settings.use_llm_mdt_agent:
                try:
                    mdt_result = run_llm_mdt_agent(
                        llm=self.llm,
                        raw_text=text,
                        entities=entities,
                        retrieved_cases=retrieved_cases,
                        kg_evidence=kg_evidence,
                        max_cases=settings.llm_mdt_max_cases,
                        max_kg=settings.llm_mdt_max_kg,
                    )
                    mdt_agent_outputs = list(mdt_result.get("agent_opinions") or [])
                    if mdt_agent_outputs:
                        agent_outputs = mdt_agent_outputs
                    critique["llm_mdt"] = mdt_result.get("critique", {})
                    mdt_report = mdt_result.get("mdt_report", {})
                except Exception as exc:
                    critique["llm_mdt_error"] = str(exc)
                    mdt_report = {"enabled": True, "source": "llm_mdt", "error": str(exc)}
        else:
            mdt_report["reason"] = "multi_agent_disabled"
        ...
        selected_baseline_modes = _normalize_baseline_modes(baseline_modes)
        baseline_match_rates = _filter_baseline_match_rates(baseline_match_rates, selected_baseline_modes)
        ...
        report = FinalReport(
            ...
            raw_baseline_result={
                ...
                "selected_baseline_modes": selected_baseline_modes,
    ...
    def _kg_disease_symptoms(
        ...
        for row in kg_evidence:
            ...
            if not disease:
                continue
            is_external = _is_external_kg_evidence(row)
            category = str(row.get("relation_category") or "")
            relation_text = _norm_text(" ".join([str(row.get("relation", "")), str(row.get("tail", ""))]))
            is_symptom = (not is_external) and (
                category == "symptom" or any(token in relation_text for token in ["symptom", "cough", "pain", "fever"])
            )
            ...
            if is_external:
                item["external_evidence_count"] = int(item.get("external_evidence_count", 0) or 0) + 1
            if is_symptom:
    ...
    def _summary_markdown(
        ...
        agent_outputs: List[Dict[str, Any]],
        llm_response: str,
        mdt_report: Optional[Dict[str, Any]] = None,
    ) -> str:
        ...
        baseline_heading = " / ".join(str(row.get("mode")) for row in baseline_match_rates if row.get("mode")) or "B0 / B1 / B2"
        kg_symptom_lines = []
        for item in kg_disease_symptoms[:5]:
            external_count = int(item.get("external_evidence_count", 0) or 0)
            if external_count:
                kg_symptom_lines.append(
                    f"- {_zh_label(item.get('disease'))}：命中 {external_count} 条外部病例弱证据；"
                    "这类证据只作为相似病例补充，不作为知识图谱强症状规则。"
                )
            else:
        ...
        for idx, case in enumerate(retrieved_cases[:3], start=1):
            label = _display_case_label(case)
        ...
        for idx, item in enumerate(kg_evidence[:5], start=1):
            kg_lines.append(_format_kg_evidence_line(idx, item))
        agent_lines = []
        ...
        return "\n".join(
            [
                ...
                "",
                f"### 三、{baseline_heading} 匹配率",
                "\n".join(baseline_lines) if baseline_lines else "未选择 B0 / B1 / B2 输出模式。",
                "",
                "### 四、相似病例证据",
                ...
                "",
                *_optional_mdt_markdown(mdt_report),
                "### 八、安全提示",
...
def _optional_mdt_markdown(mdt_report: Optional[Dict[str, Any]]) -> List[str]:
    if not mdt_report or not mdt_report.get("enabled"):
        return []
    if mdt_report.get("error"):
        return [
            "### LLM-MDT 会诊报告",
            f"LLM-MDT 调用失败：{mdt_report.get('error')}",
            "",
        ]

    final = mdt_report.get("final_report") if isinstance(mdt_report.get("final_report"), dict) else {}
    diagnosis = _zh_label(final.get("final_diagnosis") or "未生成明确 MDT 结论")
    risk_level = _zh_label(final.get("final_risk_level") or "unknown")
    confidence = _safe_float(final.get("final_confidence"), 0.0)
    reasons = final.get("diagnostic_consensual_reasons") or []
    if isinstance(reasons, str):
        reasons = [reasons]
    reason_text = "；".join(_zh_label(item) for item in reasons[:5]) or "未返回共识依据"
    summary = _zh_label(final.get("mdt_consultation_summary") or "")
    plan = _zh_label(final.get("clinical_intervention_plan") or "")

    lines = [
        f"- MDT 结论：{diagnosis}",
        f"- MDT 风险等级：{risk_level}",
        f"- MDT 置信度：{confidence:.2f}",
        f"- 共识依据：{reason_text}",
    ]
    if summary:
        lines.append(f"- 会诊摘要：{summary}")
    if plan:
        lines.append(f"- 建议方向：{plan}")
    return ["### LLM-MDT 会诊报告", "\n".join(lines), ""]


def _normalize_baseline_modes(value: Any) -> List[str]:
    default_modes = ["B0", "B1", "B2"]
    if value is None or value == "":
        return default_modes
    if isinstance(value, str):
        normalized_value = value.replace("+", ",").replace("/", ",").replace("|", ",").replace("，", ",")
        raw_items = [item.strip() for item in normalized_value.split(",")]
    ...
    for item in raw_items:
        mode = item.upper().replace(" ", "")
        if mode in {"ALL", "B0B1B2"}:
            return default_modes
        if mode in default_modes and mode not in selected:
            selected.append(mode)
    return selected or default_modes


def _filter_baseline_match_rates(
    baseline_match_rates: List[Dict[str, Any]],
    selected_modes: List[str],
) -> List[Dict[str, Any]]:
    selected = set(_normalize_baseline_modes(selected_modes))
    return [row for row in baseline_match_rates if str(row.get("mode", "")).upper() in selected]
...
_ZH_EXACT_TERMS = {
    ...
    "medcase_reasoning": "医学推理病例库",
    "open_patients": "开放患者病例库",
    "pmc_patients": "PMC 病例库",
    "external_case_evidence": "外部病例证据库",
    "Subarachnoid hemorrhage": "蛛网膜下腔出血",
    "subarachnoid_hemorrhage": "蛛网膜下腔出血",
    "N/A": "无",
...
def _is_external_kg_evidence(row: Dict[str, Any]) -> bool:
    relation = str(row.get("relation") or "")
    source = str(row.get("source") or "")
    metadata = row.get("metadata") if isinstance(row.get("metadata"), dict) else {}
    return (
        "外部病例弱证据" in relation
        or source == "external_case_evidence"
        or str(metadata.get("evidence_level") or "") == "weak_case_evidence"
    )


def _format_kg_evidence_line(idx: int, item: Dict[str, Any]) -> str:
    if _is_external_kg_evidence(item):
        metadata = item.get("metadata") if isinstance(item.get("metadata"), dict) else {}
        source = str(metadata.get("original_source") or item.get("source") or "外部病例库").strip()
        case_id = str(metadata.get("case_id") or "").strip()
        case_part = f"，case_id={case_id}" if case_id else ""
        return (
            f"{idx}. 外部病例弱证据（来源：{_zh_label(source)}{case_part}）："
            "已作为补充检索证据参与分析，页面报告不展开原始长病例文本。"
        )
    return (
        f"{idx}. {_zh_label(item.get('head', ''))} --{_zh_label(item.get('relation', ''))}--> "
        f"{_short_zh_text(_zh_label(item.get('tail', '')), max_chars=120)}"
    )


def _display_case_label(case: Dict[str, Any]) -> str:
    raw_label = str(case.get("diagnosis") or case.get("title") or case.get("case_id") or "").strip()
    label = _zh_label(raw_label)
    if _is_external_case(case) and _contains_untranslated_english(label):
        source = _zh_label(case.get("source") or (case.get("metadata") or {}).get("source") or "外部病例库")
        return f"外部相似病例（来源：{source}）"
    return label or "相似病例"


def _is_external_case(case: Dict[str, Any]) -> bool:
    source = str(case.get("source") or "").strip()
    case_id = str(case.get("case_id") or "").strip()
    return bool(source and source != "ddxplus_cases") or case_id.startswith(("medcase_reasoning:", "open_patients:", "pmc_patients:"))


def _contains_untranslated_english(text: Any) -> bool:
    value = str(text or "")
    return bool(re.search(r"[A-Za-z]{3,}", value))


def _short_zh_text(text: Any, max_chars: int = 120) -> str:
...
def _rule_based_conclusion(entities: List[Dict[str, Any]]) -> Dict[str, Any] | None:
    ...
    if names & {_norm_text("收缩压"), _norm_text("舒张压"), "sbp", "dbp"}:
        ...
        return _blood_pressure_conclusion(sbp=sbp, dbp=dbp)
    if names & {"glu", "hba1c"}:
        return {
...
def _blood_pressure_conclusion(sbp: float | None, dbp: float | None) -> Dict[str, Any]:
    findings: List[str] = []
    has_high = False
    has_low = False
    ...
    if sbp is not None:
        if sbp > 140:
            has_high = True
            findings.append(f"收缩压 {sbp:g} mmHg（升高）")
        elif sbp < 90:
            has_low = True
            findings.append(f"收缩压 {sbp:g} mmHg（偏低）")
        else:
            findings.append(f"收缩压 {sbp:g} mmHg")

    if dbp is not None:
        if dbp > 90:
            has_high = True
            findings.append(f"舒张压 {dbp:g} mmHg（升高）")
        elif dbp < 60:
            has_low = True
            findings.append(f"舒张压 {dbp:g} mmHg（偏低）")
        else:
            findings.append(f"舒张压 {dbp:g} mmHg")

    if has_high and has_low:
        primary = "血压异常（升高与偏低并存），需进一步评估"
        confidence = 0.76
    elif has_high:
        primary = "血压升高 / 高血压风险"
    ...
    return {
        ...
        "basis": "发现异常血压指标：" + ("、".join(findings) if findings else "血压不在参考范围内"),
