...
The retriever loads the Chinese JSONL KG by default. It can still optionally
merge evidence from a prebuilt ``vector_db/ddxplus_kg`` vector store, but the
FastAPI path disables that vector dependency when Chinese data is enabled.
"""
...
DEFAULT_KG_PATH = "./data_zh/kg/combined_kg_zh.jsonl"
...
def classify_relation(relation: object, head: object = "", tail: object = "") -> str:
    ...
    if "外部病例弱证据" in str(relation or "") or "external case evidence" in text:
        return "case_evidence"
    if any(term in text for term in ["differential", "鉴别"]):
        return "differential"
    if any(term in text for term in ["symptom", "symptomatology", "anamnesis", "cough", "pain", "fever", "症状", "表现", "咳嗽", "疼痛", "发热"]):
        return "symptom"
    if any(term in text for term in ["risk", "factor", "exposure", "history", "travel", "smoking", "风险", "危险", "暴露", "病史", "吸烟"]):
        return "risk_factor"
    if any(term in text for term in ["test", "lab", "blood", "glucose", "alt", "ast", "ldl", "hba1c", "检查", "检验", "血压", "血糖", "血脂"]):
        return "test_indicator"
    if any(term in text for term in ["treatment", "drug", "medication", "therapy", "治疗", "用药", "药物", "疗法"]):
        return "treatment"
    if any(term in text for term in ["complication", "causes", "associated", "并发", "导致", "相关"]):
